...
/*	devdbg(dev,"qf9700_mdio_write() phy_id=0x%02x, loc=0x%02x, val=0x%04x",
	       phy_id, loc, val); */
	dev_dbg(&dev->udev->dev, "qf9700_mdio_write() phy_id=0x%02x, loc=0x%02x, val=0x%04x",
               phy_id, loc, val);
...
//	.get_strings    = usbnet_get_strings, // Maybe this replacement is also not needed
//	.set_settings	= usbnet_set_settings, - looks like no longer needed
...
// Add other members of net_device_ops if needed
	};
// Later in your code where you initialize your network device
